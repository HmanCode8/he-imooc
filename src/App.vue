<template>
	<div class="w-full h-full flex flex-col">
		<div class="bg-slate-800 text-[#e4f9fe] text-center p-2 flex justify-between items-center ">
			<!-- 日期展示 ：当前时间年月日，星期 天气-->
			<div class="date">
				{{ new Date().toLocaleDateString() }} {{ new Date().toLocaleTimeString() }} {{ data.weather }}
			</div>
			<div class="title font-serif text-3xl font-bold">盐城综合监管系统</div>
			<!-- 用户信息展示 ：用户名、角色、部门-->
			<div class="user flex">

				<div class="name">用户名：{{ data.username }}</div>
				<div class="role">角色：{{ data.role }}</div>
				<div class="department">部门：{{ data.department }}</div>
			</div>
		</div>
		<div class="">
