/*
 *  SceneGISHeatmap.js v0.1 | SceneGIS Heatmap Library
 *  
...
(function (window) {
	...
	function define_CesiumHeatmap() {
		...
		/*  Create a SceneGISHeatmap instance
		 *
		 *  Scenegis:  the CesiumWidget or Viewer instance
		 ...
		 */
		SceneGISHeatmap.create = function (Scenegis, bb, options) {
			var instance = new CHInstance(Scenegis, bb, options);
			return instance;
		};

		SceneGISHeatmap._getContainer = function (width, height, id) {
		...
		SceneGISHeatmap._getImageryProvider = function (instance) {
			//var n = (new Date()).getTime();
			var d = instance._heatmap.getDataURL();
			//console.log("Create data URL: " + ((new Date()).getTime() - n));

			//var n = (new Date()).getTime();
			var imgprov = new SceneGIS.SingleTileImageryProvider({
				url: d,
				rectangle: instance._rectangle
			});
			//console.log("Create imageryprovider: " + ((new Date()).getTime() - n));

			imgprov._tilingScheme = new SceneGIS.WebMercatorTilingScheme({
				rectangleSouthwestInMeters: new SceneGIS.Cartesian2(instance._mbounds.west, instance._mbounds.south),
				rectangleNortheastInMeters: new SceneGIS.Cartesian2(instance._mbounds.east, instance._mbounds.north)
			});

			return imgprov;
		};

		SceneGISHeatmap._getID = function (len) {
			var text = "";
		...
		var WMP = new SceneGIS.WebMercatorProjection();

		/*  Convert a WGS84 location into a mercator location
		 *
		 *  p: the WGS84 location like {x: lon, y: lat}
		 */
		SceneGISHeatmap.wgs84ToMercator = function (p) {
			var mp = WMP.project(SceneGIS.Cartographic.fromDegrees(p.x, p.y));
			return {
				x: mp.x,
				y: mp.y
			};
		 ...
		 *  bb: the WGS84 bounding box like {north, east, south, west}
		 */
		SceneGISHeatmap.wgs84ToMercatorBB = function (bb) {
			var sw = WMP.project(SceneGIS.Cartographic.fromDegrees(bb.west, bb.south));
			var ne = WMP.project(SceneGIS.Cartographic.fromDegrees(bb.east, bb.north));
			return {
				north: ne.y,
				east: ne.x,
				south: sw.y,
		 ...
		 */
		SceneGISHeatmap.mercatorToWgs84 = function (p) {
			var wp = WMP.unproject(new SceneGIS.Cartesian3(p.x, p.y));
			return {
				x: wp.longitude,
				y: wp.latitude
			};
		 ...
		 */
		SceneGISHeatmap.mercatorToWgs84BB = function (bb) {
			var sw = WMP.unproject(new SceneGIS.Cartesian3(bb.west, bb.south));
			var ne = WMP.unproject(new SceneGIS.Cartesian3(bb.east, bb.north));
			return {
		 ...
		 */
		SceneGISHeatmap.deg2rad = function (d) {
		 ...
		 */
		SceneGISHeatmap.rad2deg = function (r) {
			var d = r / (Math.PI / 180.0);
			return d;
		};

		return SceneGISHeatmap;
	}

	if (typeof(SceneGISHeatmap) === 'undefined') {
		window.SceneGISHeatmap = define_CesiumHeatmap();
	} else {
		console.log("SceneGISHeatmap already defined.");
	}
})(window);

/*  Initiate a SceneGISHeatmap instance
 *
...
function CHInstance(c, bb, o) {
	...
	this._cesium = c;
	this._options = o;
	this._id = SceneGISHeatmap._getID();

	this._options.gradient = ((this._options.gradient) ? this._options.gradient : SceneGISHeatmap.defaults.gradient);
	this._options.maxOpacity = ((this._options.maxOpacity) ? this._options.maxOpacity : SceneGISHeatmap.defaults.maxOpacity);
	this._options.minOpacity = ((this._options.minOpacity) ? this._options.minOpacity : SceneGISHeatmap.defaults.minOpacity);
	this._options.blur = ((this._options.blur) ? this._options.blur : SceneGISHeatmap.defaults.blur);

	this._mbounds = SceneGISHeatmap.wgs84ToMercatorBB(bb);
	this._setWidthAndHeight(this._mbounds);

	this._options.radius = Math.round((this._options.radius) ? this._options.radius : ((this.width > this.height) ? this.width / SceneGISHeatmap.defaults.radiusFactor : this.height / SceneGISHeatmap.defaults.radiusFactor));

	this._spacing = this._options.radius * SceneGISHeatmap.defaults.spacingFactor;
	...
	this.bounds = SceneGISHeatmap.mercatorToWgs84BB(this._mbounds);

	this._rectangle = SceneGIS.Rectangle.fromDegrees(this.bounds.west, this.bounds.south, this.bounds.east, this.bounds.north);
	this._container = SceneGISHeatmap._getContainer(this.width, this.height, this._id);
...
CHInstance.prototype.wgs84PointToHeatmapPoint = function (p) {
	return this.mercatorPointToHeatmapPoint(SceneGISHeatmap.wgs84ToMercator(p));
};
...
CHInstance.prototype._setWidthAndHeight = function (mbb) {
	...
	if (this.width > this.height && this.width > SceneGISHeatmap.defaults.maxCanvasSize) {
		this._factor = this.width / SceneGISHeatmap.defaults.maxCanvasSize;

		if (this.height / this._factor < SceneGISHeatmap.defaults.minCanvasSize) {
			this._factor = this.height / SceneGISHeatmap.defaults.minCanvasSize;
		}
	} else if (this.height > this.width && this.height > SceneGISHeatmap.defaults.maxCanvasSize) {
		this._factor = this.height / SceneGISHeatmap.defaults.maxCanvasSize;

		if (this.width / this._factor < SceneGISHeatmap.defaults.minCanvasSize) {
			this._factor = this.width / SceneGISHeatmap.defaults.minCanvasSize;
		}
	} else if (this.width < this.height && this.width < SceneGISHeatmap.defaults.minCanvasSize) {
		this._factor = this.width / SceneGISHeatmap.defaults.minCanvasSize;

		if (this.height / this._factor > SceneGISHeatmap.defaults.maxCanvasSize) {
			this._factor = this.height / SceneGISHeatmap.defaults.maxCanvasSize;
		}
	} else if (this.height < this.width && this.height < SceneGISHeatmap.defaults.minCanvasSize) {
		this._factor = this.height / SceneGISHeatmap.defaults.minCanvasSize;

		if (this.width / this._factor > SceneGISHeatmap.defaults.maxCanvasSize) {
			this._factor = this.width / SceneGISHeatmap.defaults.maxCanvasSize;
		}
	}
...
CHInstance.prototype.updateLayer = function () {
	...
	if (SceneGISHeatmap.defaults.useEntitiesIfAvailable && this._cesium.entities) {
		if (this._layer) {
			this._cesium.entities.remove(this._layer);
		}

		// Work around issue with material rendering in SceneGIS
		// provided by https://github.com/criis
		let material = new SceneGIS.ImageMaterialProperty({
			image: this._heatmap._renderer.canvas,
		});
		if (SceneGIS.VERSION >= "1.21") {
			material.transparent = true;
		} else if (SceneGIS.VERSION >= "1.16") {
			material.alpha = 0.99;
		}
		...
		this._layer = this._cesium.scene.imageryLayers.addImageryProvider(SceneGISHeatmap._getImageryProvider(this));
	}
};
