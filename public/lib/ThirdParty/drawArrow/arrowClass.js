...
var StraightArrow = function (viewer) {
	...
	this.viewer = viewer;
	this.handler = new SceneGIS.ScreenSpaceEventHandler(this.viewer.scene.canvas);
	this.pointImageUrl = "img/point.png";
	this.fillMaterial = SceneGIS.Color.fromCssColorString('#0000FF').withAlpha(0.8);
	this.outlineMaterial = new SceneGIS.PolylineDashMaterialProperty({
		dashLength: 16,
		color: SceneGIS.Color.fromCssColorString('#f00').withAlpha(0.7)
	});
	this.positions = [];
...
StraightArrow.prototype = {
	...
	startDraw: function () {
		...
		this.handler.setInputAction(function (evt) { //移动时绘制面
			...
			if ($this.positions.length >= 2) {
				if (!SceneGIS.defined($this.arrowEntity)) {
					$this.positions.push(cartesian);
					$this.arrowEntity = $this.showArrowOnMap($this.positions,$this.planeView);
				} else {
					$this.positions.pop();
					$this.positions.push(cartesian);
				}
	...
	startModify: function () { //修改箭头
		...
		var $this = this;
		this.clickStep = 0;
		if (!this.modifyHandler) this.modifyHandler = new SceneGIS.ScreenSpaceEventHandler(this.viewer.scene.canvas);
		this.modifyHandler.setInputAction(function (evt) { //单机开始绘制
			var pick = $this.viewer.scene.pick(evt.position);
			if (SceneGIS.defined(pick) && pick.id) {
				$this.clickStep++;
				if (!pick.id.objId)
	...
	createByData: function (data) { //通过传入的经纬度数组 构建箭头
		...
		for (var i = 0; i < data.length; i++) {
			var cart3 = SceneGIS.Cartesian3.fromDegrees(data[i][0], data[i][1]);
			arr.push(cart3);
		}
		this.positions = arr;
	...
	showArrowOnMap: function(positions,planeView) {
		var $this = this;
		var update = function () {
			...
			var p1 = positions[1];
			var p2 = positions[2];
			var firstPoint,endPoints;
			if(planeView === true){
				firstPoint = [p1.x,p1.y];
				endPoints = [p2.x,p2.y];
			}else{
				firstPoint = $this.cartesianToLatlng(p1);
				endPoints = $this.cartesianToLatlng(p2);
			}
			var arrow = [];
			var res = xp.algorithm.fineArrow([firstPoint[0], firstPoint[1]], [endPoints[0], endPoints[1]], planeView);
			var index = JSON.stringify(res).indexOf("null");
			if (index != -1) return [];
			for (var i = 0; i < res.length; i++) {
				var c3 = new SceneGIS.Cartesian3(res[i].x, res[i].y, res[i].z);
				arrow.push(c3);
			}
			return new SceneGIS.PolygonHierarchy(arrow);
		}
		return this.viewer.entities.add({
			polygon: new SceneGIS.PolygonGraphics({
				hierarchy: new SceneGIS.CallbackProperty(update, false),
				show: true,
				fill: true,
	...
	cartesianToLatlng: function (cartesian) {
		...
		var lat = SceneGIS.Math.toDegrees(latlng.latitude);
		var lng = SceneGIS.Math.toDegrees(latlng.longitude);
		return [lng, lat];
	}
}
...
var AttackArrow = function (viewer) {
	...
	this.pointImageUrl = "img/point.png";
	this.handler = new SceneGIS.ScreenSpaceEventHandler(this.viewer.scene.canvas);
	this.fillMaterial = SceneGIS.Color.RED.withAlpha(0.8);
	this.outlineMaterial = new SceneGIS.PolylineDashMaterialProperty({
		dashLength: 16,
		color: SceneGIS.Color.fromCssColorString('#f00').withAlpha(0.7)
	});
...
var PincerArrow = function (viewer) {
	...
	this.handler = new SceneGIS.ScreenSpaceEventHandler(this.viewer.scene.canvas);
	this.pointImageUrl = "img/point.png";
	this.fillMaterial = SceneGIS.Color.YELLOW.withAlpha(0.8);
	this.outlineMaterial = new SceneGIS.PolylineDashMaterialProperty({
		dashLength: 16,
		color: SceneGIS.Color.fromCssColorString('#f00').withAlpha(0.7)
	});
...
PincerArrow.prototype = {
	...
	showArrowOnMap: function(positions, planeView) {
		...
		var update = function () {
			...
			var res = xp.algorithm.doubleArrow(lnglatArr, planeView);
			var returnData = [];
			var index = JSON.stringify(res.polygonalPoint).indexOf("null");
			if (index == -1) returnData = res.polygonalPoint;
			return new SceneGIS.PolygonHierarchy(returnData);
...
function getCatesian3FromPX(px, viewer) {
	...
	for (var i = 0; i < picks.length; i++) {
		if ((picks[i] && picks[i].primitive) || picks[i] instanceof SceneGIS.Cesium3DTileFeature) { //模型上拾取
			isOn3dtiles = true;
		}
	}
	if (isOn3dtiles) {
