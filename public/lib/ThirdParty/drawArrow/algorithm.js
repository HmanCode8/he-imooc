...
xp.algorithm = {},
xp.algorithm.doubleArrow = function (inputPoint, planeView) {
    this.connPoint = null;
    ...
    if (!(2 > t)) {
        ...
        if(planeView === true){
            for(let i=0;i<newArray.length-1;i+=2){
                let pos = new SceneGIS.Cartesian3(newArray[i],newArray[i+1],0);
                newArray2.push(pos);
            }
        }
        result.controlPoint = [o, e, r, this.tempPoint4, this.connPoint];
        result.polygonalPoint = planeView === true? newArray2:SceneGIS.Cartesian3.fromDegreesArray(newArray);
    }
    return result;
},
xp.algorithm.threeArrow = function (inputPoint) {
    ...
    if (t >= 2) {
        ...
        result.polygonalPoint = SceneGIS.Cartesian3.fromDegreesArray(newArray);
    }
    return result;
},
...
xp.algorithm.tailedAttackArrow = function (inputPoint, planeView) {
...
xp.algorithm.fineArrow = function (tailPoint, headerPoint, planeView) {
    ...
    d = [];
    d.push(a[0], a[1], 0, p[0], p[1], 0, u[0], u[1], 0, r[0], r[1], 0, c[0], c[1], 0, h[0], h[1], 0, l[0], l[1], 0, e[0], e[1], 0);
    return planeView === true? SceneGIS.Cartesian3.unpackArray(d)
                             : SceneGIS.Cartesian3.fromDegreesArrayHeights(d);
}